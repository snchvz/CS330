...
public:
    ...
    typedef Reverse_iterator<T> revItr;
...
template<class T>
T & List<T>::operator[](unsigned int index) //O(n)
{
    int i = 0;
...
template<class T>
typename List<T>::revItr List<T>::rbegin() const //O(1)
{
    return revItr(first_link);
}

template<class T>
typename List<T>::revItr List<T>::rend() const  //O(1)
{
    return revItr(last_link);
}
    
template<class T>
List<T> List<T>::reverse()    //O(n)
{
    List<T> x;
    List<T>::iterator itr = begin();
    while (itr != end()){
...
template<class T> 
T & Reverse_iterator<T>::operator*() //O(1)
{
    return current_link->value;
}

template<class T> 
Reverse_iterator<T> & Reverse_iterator<T>::operator++() //O(1)
{
...
template<class T>
Reverse_iterator<T> Reverse_iterator<T>::operator++(int) //O(1)
{
    Link<T> * temp = current_link;
    current_link = current_link->prev_link;
    return revItr(temp);
